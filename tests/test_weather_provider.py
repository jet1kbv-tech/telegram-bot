...
def test_open_meteo_normalizes_and_caches_both_requests():
    ...
    def handler(request):
        ...
        return httpx.Response(200, json={"daily": {"time": [forecast_date.isoformat()], "temperature_2m_min": [12],
    ...
    async def scenario():
        async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
            ...
            first = await provider.get_forecast("Воронеж", forecast_date)
            second = await provider.get_forecast("Воронеж", forecast_date)
            return first, second
