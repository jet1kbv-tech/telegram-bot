...
query_context=query_type,destination,transport_type; query_purchases=status,priority,buyer,operation; query_films=status,media_type,genre,operation; query_calendar/query_afisha=date_from,date_to,target,operation; unsupported=category; no_action=без полей.

Intents: add_personal_calendar_event/add_afisha_event/add_purchase/add_movie_or_tv; update/delete_purchase, _film, _calendar_event, _afisha_event; attach_event_file; update_event_attachment/delete_event_attachment; read-only query_context/query_event_attachments/query_purchases/query_films/query_calendar/query_afisha; unsupported; no_action. Различай add, update, delete и query как ровно одно действие. Терпи разговорную грамматику, склонения, порядок слов и отсутствие пунктуации. Недостающие значения = null, а не unsupported/no_action. Для update заполняй только явно изменяемые поля; target — название, не ID. Purchase statuses planned/bought, priority только high/medium/low; film want/watched.
